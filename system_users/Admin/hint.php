      ...
      <div class="container-fluid py-1 px-3">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
            ...
            <li class="breadcrumb-item text-sm text-dark active" aria-current="page">Hint</li>
          </ol>
          <h6 class="font-weight-bolder mb-0">Hint</h6>
        </nav>
...
function time_ago($timestamp) {
    ...
    // Loop through the intervals to find the appropriate one
    foreach ($intervals as $secs => $description) {
        $diff = $time_diff / $secs;
        if ($diff < 1) {
            continue;
        }
        $diff = round($diff);
        if ($diff == 1) {
            return $diff . ' ' . $description[0] . ' ago';
        } else {
            return $diff . ' ' . $description[1] . ' ago';
        }
    }
...
// Example usage:
$timestamp = strtotime('2024-02-12 08:30:00'); // Example last seen timestamp
echo time_ago($timestamp); // Output: 6 hours ago
